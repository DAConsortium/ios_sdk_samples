...
#import "DACSDKMA_SampleBCOVPlayer_ObjectiveC-Swift.h"
...
#pragma mark - BCOVSDK
static NSString * const kAccountID = @"1234567899001";
...
- (void)dacsdkmaAdsLoader:(DACSDKMAAdsLoader *)loader didLoad:(DACSDKMAAdsLoadedData *)adsLoadedData {
    ...
    [self.dacAdsManager loadWithCompletion: ^(BOOL result) {
        if (result) {
            self.dacAdController = [[DACSDKMAAdController alloc] initWithAdsManager:self.dacAdsManager];
        }
    }];
...
- (void)dacsdkmaAdsLoader:(DACSDKMAAdsLoader *)loader didFail:(DACSDKMAAdError *)adError {
    NSLog(@"Error: dacSdkMaAdsLoader:(loader) didFaile(adError) = %@", adError.message);
