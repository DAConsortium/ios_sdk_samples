...
SWIFT_CLASS("_TtC11DACSDKPingV10DACSDKInfo")
...
+ (NSString * _Nonnull)devicePlatformName SWIFT_WARN_UNUSED_RESULT;
/// デバイスのモデル名を返します。
///
/// returns:
...
/// Log出力クラス
SWIFT_CLASS("_TtC11DACSDKPingV12DACSDKLogger")
...
+ (DACSDKPingV * _Nonnull)shared SWIFT_WARN_UNUSED_RESULT;
...
+ (DACSDKPingVApplicationDataCenter * _Nonnull)shared SWIFT_WARN_UNUSED_RESULT;
...
/// ユーティリティクラス
SWIFT_CLASS("_TtC11DACSDKPingV10DACSDKUtil")
@interface DACSDKUtil : NSObject
/// lowerからupperで指定した値のランダム値を返す。
+ (uint64_t)randomUInt64WithLower:(uint64_t)lower upper:(uint64_t)upper SWIFT_WARN_UNUSED_RESULT;
/// 指定されたQueueでdelay後にblockを呼び出す。
/// キャンセルする場合は、dispatchCancelにdispatch_cancelable_block_tを代入する。
+ (void (^ _Nullable)(BOOL))dispatchAfterCancelableWithDelay:(double)delay queue:(dispatch_queue_t _Nonnull)queue block:(void (^ _Nullable)(void))block SWIFT_WARN_UNUSED_RESULT;
